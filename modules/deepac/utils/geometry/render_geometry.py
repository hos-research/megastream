...
class GenerateGeodesicPoses():
    ...
    def __init__(self, maximum_body_diameter, sphere_radius, image_size, image_border_size, n_divide, device='cpu'):
        ...
        self.geodesic_points = np.loadtxt(f'{str(template_dir)}/geodesic_points_{n_divide}.txt')
...
class RenderGeometry:
    ...
    def calculate_line_distances(self, image_size, masks, sample_centers, sample_normals, pixel_to_meter):
        ...
        width = image_size[0]
        height = image_size[1]
        foreground_distance = np.zeros(shape=(n_obj, n_sample), dtype=np.float64)
        background_distance = np.zeros(shape=(n_obj, n_sample), dtype=np.float64)
